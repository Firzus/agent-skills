...
The sub-cell streaming frontier. The core skill treats the **cell** as the
...
The `t_io / t_decompress` tier — extends the core skill's IO throughput
table:
...
- **Float precision at scale** — 32-bit float (24-bit mantissa) jitters far
  ...
  looks) — the render-side twin of the core skill's floating-origin shift.
...
- **Transitions** — **dithered LOD** (ramp a screen-space dither threshold;
  ...
  skill's "cross-fade transitions" bullet.
...
- **Two nested streaming loops** — cell-level (CPU, distance/velocity-driven
  — the core skill) and sub-cell (GPU feedback-driven — Nanite pages, VT
...
details evolve fast (keep version-specific notes out of skill bodies).
