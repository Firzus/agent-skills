...
The content-creation and living-world layers the core skill omits: how a
...
- **Scatter** — **Poisson-disk sampling** (points with a minimum distance r
  ...
  foliage placements the core skill loads.
...
- **Minecraft** — a **chunk** (16×16 column, 384 tall since 1.18) *is* the
  core skill's cell, but the load step is **generate**, not read. A single
...
- **Simulation-LOD / level-of-simulation** — full sim near the player,
  **statistical/abstract sim far** (the core skill already notes Genshin
...
- **Crowd simulation** — ambient population via **spawn/despawn pools**
  (→ the core skill's "pool streamed entities, avoid alloc/free churn"),
...
- **Persist the delta from the generated baseline** — for a procedural/
  ...
  *modified* chunks; untouched chunks regenerate). The exact generalization
  of the save skill's rule: *save only authoritative state that cannot be
  deterministically reconstructed; recompute the rest.*
- **Per-cell state** — the save skill's world-state store (stable-ID flags
...
- → **Fast travel** over a procedural world: activate a source at the
  ...
  before revealing — the same gate as the core skill's "wait for
...
- → Determinism is what lets a procedural world plug into the core skill's
...
SKILL.md + store-model.md. Flags: octave/erosion numbers are art-directed;
