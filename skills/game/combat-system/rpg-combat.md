...
× Damage × E[mult]`. The melee skill's ×1.25→×1.5 crit plugs straight in.
...
- **Crowd control + diminishing returns** — CC (stun/root/slow/silence)
  ...
  directly parallels the melee skill's anti-stunlock stun-gauge escalation
...
- **Difficulty scaling** — flat multipliers (HP×2 — cheap but spongy, the
  ...
  **stunlock** (the skill's anti-stunlock + CC-DR), displayed-high-% misses.
- **Accessibility difficulty** — damage sliders/multipliers (Celeste
  ...
  skill's feel-tuning philosophy to inclusivity.
...
- **Soulslike = real-time action + RPG math**: the attack graph + Souls
  ...
  during attacks — the skill's poise/hyperarmor, just geared for) +
...
- **MMO action-combat** — the **GCD** (~1.5 s gating most abilities, both
  ...
  **telegraphs** (AoE warnings) are the MMO cousin of the skill's wind-up/
