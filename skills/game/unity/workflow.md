...
- Log through a leveled wrapper, strip verbose logs from release builds, and assert invariants with `Debug.Assert`, which compiles out of non-dev builds. A per-frame `Debug.Log` allocates a string and captures a stack trace even with the console closed.
