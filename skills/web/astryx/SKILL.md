...
5. **One color-mode owner.** The `<Theme mode>` provider owns light/dark (`mode` default `'system'`); never run a second dark-mode provider. `Theme`, `useTheme`, and `defineTheme` are all exported from **both** `@astryxdesign/core` and `@astryxdesign/core/theme` — either import path is fine (verified v0.1.7). → [theming-tokens.md](./theming-tokens.md)
